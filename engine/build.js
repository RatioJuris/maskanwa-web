...
function resolveTenantSource(slug) {
    if (
        ...
        slug.toLowerCase() === PLATFORM_SLUG
    ) {
        throw new Error(
            `Unsafe tenant slug detected: ${slug}`
        );
    }
...
function resolveTenantOutput(slug) {
    if (
        ...
        throw new Error(
            `Unsafe tenant output slug detected: ${slug}`
        );
    }
...
async function buildContent(
    ...
    const instPath = isShowcase
        ? resolvePlatformSource()
        : resolveTenantSource(slug);
